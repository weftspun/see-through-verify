...
The see-through CLI works but requires terminal usage. A native GUI makes
the tool accessible to artists and non-technical users.

## Proposal

A native macOS GUI uses ThorVG (drawing) + GLFW (window/input) + Vulkan (GPU
compute backend). The GUI shows:
...
The GLFW window and Vulkan context do not initialize yet; the ThorVG canvas
has no UI layout, the pipeline dispatch has no background thread or progress
updates, and no `.app` bundle exists.
