...
void groupnorm_kern_Thread(ComputeThreadVaryingInput* varyingInput, void* entryPointParams, void* globalParams)
{
    _groupnorm_kern(varyingInput, entryPointParams, globalParams);
}
// [numthreads(8, 8, 1)]
SLANG_PRELUDE_EXPORT
void groupnorm_kern_Group(ComputeVaryingInput* varyingInput, void* entryPointParams, void* globalParams)
{
    ...
    for (uint32_t y = 0; y < 8; ++y)
    {
        ...
        for (uint32_t x = 0; x < 8; ++x)
        {
            ...
            _groupnorm_kern(&threadInput, entryPointParams, globalParams);
        }
    }
}
// [numthreads(8, 8, 1)]
SLANG_PRELUDE_EXPORT
void groupnorm_kern(ComputeVaryingInput* varyingInput, void* entryPointParams, void* globalParams)
{
    ...
    for (uint32_t z = vi.startGroupID.z; z < vi.endGroupID.z; ++z)
    {
        ...
        for (uint32_t y = vi.startGroupID.y; y < vi.endGroupID.y; ++y)
        {
            ...
            for (uint32_t x = vi.startGroupID.x; x < vi.endGroupID.x; ++x)
            {
                ...
                groupnorm_kern_Group(&groupVaryingInput, entryPointParams, globalParams);
            }
        }
    }
}
