...
#line 28 "shaders/layernorm.slang"
struct GlobalParams_0
{
    StructuredBuffer<float> X_in_0;
    StructuredBuffer<float> G_in_0;
...
#line 13
void _layernorm_kern(void* _S1, void* entryPointParams_0, void* globalParams_1)
{
...
#line 15
    bool _S5;
    if(t_0 >= ((slang_bit_cast<GlobalParams_0*>(globalParams_1))->T_0))
    {
...
#line 16
        _S5 = d_0 >= ((&kernelContext_0)->globalParams_0->D_0);
...
#line 19
        if(k_0 < ((&kernelContext_0)->globalParams_0->D_0))
        {
        }
        else
        {
...
#line 19
        float mean_1 = mean_0 + (&kernelContext_0)->globalParams_0->X_in_0.Load(t_0 * (&kernelContext_0)->globalParams_0->D_0 + k_0);
...
#line 19
    ...
    float mean_2 = mean_0 / float(_S2->D_0);
...
#line 23
        float q_0 = (&kernelContext_0)->globalParams_0->X_in_0.Load(t_0 * _S2->D_0 + k_0) - mean_2;
...
#line 28
    *(&((&kernelContext_0)->globalParams_0->Y_out_0)[t_0 * _S2->D_0 + d_0]) = ((&kernelContext_0)->globalParams_0->X_in_0.Load(t_0 * _S2->D_0 + d_0) - mean_2) * (1.0f / (F32_sqrt((var_0 / float(_S2->D_0) + (&kernelContext_0)->globalParams_0->EPS_0)))) * (&kernelContext_0)->globalParams_0->G_in_0.Load(d_0) + (&kernelContext_0)->globalParams_0->B_in_0.Load(d_0);
    return;
}

// [numthreads(8, 8, 1)]
SLANG_PRELUDE_EXPORT
void layernorm_kern_Thread(ComputeThreadVaryingInput* varyingInput, void* entryPointParams, void* globalParams)
{
    _layernorm_kern(varyingInput, entryPointParams, globalParams);
}
// [numthreads(8, 8, 1)]
SLANG_PRELUDE_EXPORT
void layernorm_kern_Group(ComputeVaryingInput* varyingInput, void* entryPointParams, void* globalParams)
{
    ...
    for (uint32_t y = 0; y < 8; ++y)
    {
        ...
        for (uint32_t x = 0; x < 8; ++x)
        {
            ...
            _layernorm_kern(&threadInput, entryPointParams, globalParams);
        }
    }
}
// [numthreads(8, 8, 1)]
SLANG_PRELUDE_EXPORT
void layernorm_kern(ComputeVaryingInput* varyingInput, void* entryPointParams, void* globalParams)
{
    ...
    for (uint32_t z = vi.startGroupID.z; z < vi.endGroupID.z; ++z)
    {
        ...
        for (uint32_t y = vi.startGroupID.y; y < vi.endGroupID.y; ++y)
        {
            ...
            for (uint32_t x = vi.startGroupID.x; x < vi.endGroupID.x; ++x)
            {
                ...
                layernorm_kern_Group(&groupVaryingInput, entryPointParams, globalParams);
            }
        }
    }
}
